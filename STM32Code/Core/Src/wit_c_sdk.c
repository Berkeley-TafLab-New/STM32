...
//static uint16_t s_usWitRegData[REGSIZE];    //Compatible with big-endian and little-endian chips
...
{
	if(!CheckRange(uiBaudIndex,BAUD_4800,BAUD_921600))
	{
		return WIT_HAL_INVAL;
	}
	if(WitWriteReg(KEY, KEY_UNLOCK) != WIT_HAL_OK)	return  WIT_HAL_ERROR;
...
/*change Bandwidth*/
int32_t WitBandwidth(int32_t uiBaudWidth)
{	
...
/*change output rate */
int32_t WitOutputRate(int32_t uiRate)
{	
...
int32_t WitIICAddr(uint8_t ucAddr)
{	
	if(!CheckRange(ucAddr,0,0x7F))
	{
		return WIT_HAL_INVAL;
	}
	if(WitWriteReg(KEY, KEY_UNLOCK) != WIT_HAL_OK)	return  WIT_HAL_ERROR;
	p_WitDelaymsFunc(1);
	if(WitWriteReg(IICADDR, ucAddr) != WIT_HAL_OK)	return  WIT_HAL_ERROR;
	return WIT_HAL_OK;
}
